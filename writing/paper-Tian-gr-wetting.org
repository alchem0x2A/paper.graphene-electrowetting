...
#+BEGIN_EXPORT latex
\begin{tocentry}
\includegraphics[width=8.60cm]{../img/TOC.pdf}
\end{tocentry}
#+END_EXPORT
...
causes a decrease in the adhesion energy of -10 ~ -15 mJ$\cdot
...
equilibrium distance, as seen in Figure S4(a), the distance between
...
in the $\delta_{\mathrm{L}}$ and water
dipole orientation $\cos \mu$ (Figure S1) profiles, both profiles
...
Figure S2) also matches those observed in $\rho_{\mathrm{L}}$,
...
layer, when graphene becomes more n-doped (see Figure S4(d)).  As a
...
S4(d)), which could be a combined result of the graphene-water
...
atoms will increase due to Coulombic repulsions.  Note that the
...
e \cdot \mathrm{cm}^{-2}$ (Figure S4(b)). Here we propose a simple
...
$\sigma_{\mathrm{Coul}}^{\mathrm{int}}$ (see the discussion in
Supporting Information). We show that in the n-doped graphene system
...
Although the MD simulation results show that Coulombic interaction
...
other types of 2D materials, as the intensity of vdW interactions and
surface charge polarization can vary greatly. First, the microscopic vdW
and Coulombic interactions on 2D materials depend on the lattice
constant and atomic density. In addition, for homoatomic 2D materials
such as graphene, silicene, germanene and phosphorene, their surface
charge polarization is negligible compared to heteroatomic 2D
materials, for instance TMDC. It has been shown that, for TMDCs, the value of
$\gamma_{\mathrm{2D-L}}$ is already largely determined by the
Coulombic interactions in the charge-neutral systems
...
\Phi_{\mathrm{LJ}}$ and $\Delta \Phi_{\mathrm{Coul}}$ as functions of
$\sigma_{\mathrm{2D}}$ in n- and p-doped regimes, respectively (see
Supporting Information Figure S3), and use the fitting results to
contrust the $\Delta \Phi-\sigma_{\mathrm{2D}}$ relation. We find that
the reorientation effect is more predominant than the EDL effect,
...
\end{acknowledgement}
*************** TODO The acknowledgement from other coauthors
*************** END
